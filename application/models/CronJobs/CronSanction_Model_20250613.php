...
class CronSanction_Model extends CronCommon_Model {
    ...
    public function get_lead_new_list($user_type = null, $orderBy = null) {
        ...
        if ($user_type == 'REPEAT') {
            ...
            //$sql .= " WHERE LD.status='LEAD-NEW' AND LD.pancard IS NOT NULL AND (LD.lead_screener_assign_user_id IS NULL OR LD.lead_screener_assign_user_id=0)  AND `LD`.`lead_is_mobile_verified` = 1 AND LD.first_name IS NOT NULL AND `LD`.`lead_active` = 1 AND `LD`.`lead_deleted` = 0 and LD.created_on > '2024-08-20 23:53:00' and `LD`.`source` = 'ARDSALARYONTIME' ORDER BY LD.monthly_salary_amount DESC, LD.lead_entry_date DESC;";
        }
    ...
    public function get_users_lead_list($user_type = 0) {
        ...
        $sql  = "SELECT
                ...
                WHERE
                    ...
                    AND U.user_id IN(202,203,204,205,206,207,8,138,140,141)
    ...
    public function get_users_lead_hold_list() {
        ...
        $sql  = "SELECT
                ...
                WHERE
                    ...
                    AND UR.user_role_active = 1
                    AND UR.user_role_product_id = 1
                    AND U.user_id IN(204,207,205,203,208,214,213,202,218,220,219,223,257,241)
                GROUP BY
                    U.user_id
    ...
    public function get_credit_users_lead_list($allocation_type = 0) {
        ...
        if ($allocation_type == 2) {
            // $AllowUsersList = "37,6,55,56,65,21,23,73, 24,7,8,19, 119, 114, 115, 113";
            $AllowUsersList = "69,61,7,48,65,62,55,23,19,79,56,72,21,8,57,24,119,74,6,135,136,37,151";
        } elseif ($allocation_type == 1) {
            // $AllowUsersList = "21, 72, 79, 48, 57, 19, 62, 23, 62, 73, 74, 20, 32, 77, 60, 61, 58, 124,128,129,130,131,135,136, 137, 307";
            $AllowUsersList = "20,113,115,131,124,128,58 ,73,130,114,129,77,60,32,146";
            // $AllowUsersList = "124,128,129,130,131,135,136,137";
        } else {
            $AllowUsersList = "";
        }
        ...
        $sql  = "SELECT
                ...
                LEFT JOIN
                    ...
                    AND LD.lead_status_id IN (5) AND LD.user_type = 'NEW')
                WHERE
                    U.user_id = UR.user_role_user_id
                    AND UR.user_role_type_id = 3
                    AND U.user_allocation_type_id = $allocation_type
    ...
    public function get_application_hold($type = 0, $set_hours = 0) {
        $return_array = ['status' => 0];
        ...
        $type_filter = "";
        if ($type == 1) {
            $type_filter = "AND monthly_salary_amount < 50000";
        } elseif ($type == 2) {
            $type_filter = "AND monthly_salary_amount >= 50000)";
        }
    ...
    public function get_credit_users_repeat_list() {
        $current_date = date("Y-m-d");
        $return_array = ['status' => 0, 'data' => []];
        // $AllowUsersList = "AND U.user_id IN (144,176,88,161,93,142,62,18,192,191,212,98,61,231,84)"; // Repeat Users
        // $AllowUsersList = "AND U.user_id IN (144,176,88,161,93,142,62,18,192,191,212,98,61,231,218, 213, 205, 208, 84)";
        $AllowUsersList = "AND U.user_id IN (144,176,88,161,93,142,62,18,192,191,212,98,61,231,84,190)"; // Repeat Users
    ...
    public function get_rejection_application_list($list = [], $set_hours = 36) {
        $return_array = ['status' => 0, 'data' => []];

        $tempDetails = $this->db->query("SELECT
                    TIMESTAMPDIFF(HOUR, lead_credit_assign_datetime, NOW()) AS 'hold_hour',
                    lead_id
                FROM
                    leads
                WHERE
                    TIMESTAMPDIFF(HOUR, lead_credit_assign_datetime, NOW()) >= $set_hours
                    AND lead_status_id IN(5, 6)
                    AND user_type = 'NEW'
                    AND lead_credit_assign_user_id IN(" . implode(",", $list) . ")
                ORDER BY
                    hold_hour ASC");
    ...
    public function get_rejection_lead_list($list = [], $set_hours = 36) {
        $return_array = ['status' => 0, 'data' => []];

        $tempDetails = $this->db->query("SELECT
                        TIMESTAMPDIFF(HOUR, lead_screener_assign_datetime, NOW()) AS 'hold_hour',
                        lead_id
                    FROM
                        leads
                    WHERE
                        TIMESTAMPDIFF(HOUR, lead_screener_assign_datetime, NOW()) >= $set_hours
                        AND lead_status_id IN(2, 3)
                        AND user_type = 'NEW'
                        AND lead_screener_assign_user_id IN(" . implode(",", $list) . ")
                    ORDER BY
                        hold_hour ASC");
    ...
    public function get_rejection_lead_new_bucket_list($set_hours = 72) {
        $return_array = ['status' => 0, 'data' => []];

        $tempDetails = $this->db->query("SELECT
                        TIMESTAMPDIFF(HOUR, created_on, NOW()) AS 'hold_hour',
                        lead_id
                    FROM
                        leads
                    WHERE
                    	TIMESTAMPDIFF(HOUR, created_on, NOW()) >= $set_hours

...
class Report_Model extends CI_Model {
    ...
    public function DisbursalExecutiveWiseReport($fromDate, $toDate) {
        if (!empty($fromDate)) {
            ...
            $q = "SELECT
                ...
                FROM
                    ...
                    LEFT JOIN credit_analysis_memo CAM ON (
                        ...
                        AND LD.lead_final_disbursed_date >= '$fromDate'
                        AND LD.lead_final_disbursed_date <= '$toDate'
                    )
                WHERE
            ...
            if ($result) {
                ...
                foreach ($result as $row) {
                    $team = (strpos($row['credit_head'], 'SUNIL') !== false) ? 'Team A' : 'Team B';
                ...
                <head>
                    ...
                    <style>
                        ...
                        th {
                            background: linear-gradient(to bottom, #001bff, #434bff);
                            color: white;
                            font-weight: bold;
                            font-size: 16px;
                        }
                    ...
                    <table>
                        <thead>
                            <tr class="team-header">
                                <th colspan="7">TEAM A</th>
                                <th colspan="7">TEAM B</th>
                            </tr>
                            <tr>
                                <th>Name</th>
                                <th>Fresh</th>
    ...
    public function DashboardDisbursalExecutiveWiseReport_nouse($fromDate, $toDate) {
        if (!empty($fromDate)) {
            $fromDate = date('Y-m-d', strtotime($fromDate));
            $toDate = date("Y-m-d", strtotime($toDate));
        ...
        if (!empty($fromDate)) {
            ...
            if ($result) {
                ...
                $html = '<table>
                        ...
                        <tbody style="background-image: url(\'https://salaryontime.in/public/front/img/company_logo.png\') !important;">';
    ...
    public function exportProcessTATModel($from_Date) {
        ...
        $sql = "WITH ordered_followups AS (
                                    SELECT
                                        LD.lead_id,
                                        LD.lead_recommend_sendback_flag,
                                        LD.audit_send_back,
                                        LF.status,
                                        MIN(LF.created_on) AS date_time,
                                        U.name,
                                        ROW_NUMBER() OVER (PARTITION BY LD.lead_id ORDER BY MIN(LF.created_on)) AS rn
                                    FROM
                                        leads LD
                                    INNER JOIN
                                        lead_followup LF ON LD.lead_id = LF.lead_id
                                    LEFT JOIN
                                        users U ON U.user_id = LF.user_id
                                    WHERE
                                        LF.lead_followup_status_id IN (10, 12, 13, 25, 30, 35, 37, 44, 45, 46, 47, 14, 16, 17, 18, 19)
                                        AND DATE(LD.lead_credit_recommend_datetime) = '$fromDate'
                                        AND LD.user_type = 'NEW'
                                    GROUP BY
                                        LD.lead_id, LF.lead_followup_status_id
                                ),
                                diffs AS (
                                    SELECT
                                        o.*,
                                        TIMESTAMPDIFF(SECOND,
                                            LAG(o.date_time) OVER (PARTITION BY o.lead_id ORDER BY o.date_time),
                                            o.date_time
                                        ) AS diff_seconds,
                                        TIMESTAMPDIFF(HOUR,
                                            LAG(o.date_time) OVER (PARTITION BY o.lead_id ORDER BY o.date_time),
                                ...
                                final AS (
                                    SELECT
                                        ...
                                        d.hour_diff_prev,
                                        d.minute_diff_prev,
                                        SEC_TO_TIME(d.diff_seconds) AS time_diff_prev,
                                        d.lead_recommend_sendback_flag,
                                        d.audit_send_back,
                                        d.diff_seconds
                                    FROM diffs d
                                )
                                SELECT
                                    f.*,
                                    SEC_TO_TIME(SUM(IFNULL(f.diff_seconds, 0)) OVER (PARTITION BY f.lead_id)) AS total_time_per_lead
                                FROM final f
                                ORDER BY f.lead_id, f.date_time ASC";
        ...
        foreach ($rows as $row) {
            $lead_id = $row['lead_id'];
            $status = $row['status'];
            $final_report[$lead_id][$status] = $row;
            $final_report[$lead_id]['time_diff_prev'] += $row['time_diff_prev'] ?? 0;
            $final_report[$lead_id]['total_time_per_lead'] += $row['total_time_per_lead'] ?? 0;
        }
        ...
        // Calculate width
        $total_columns = 2 + (count($statusOrder) - 1) * 2 + 1;
        $column_width = round(100 / $total_columns, 2) + 0.01;

        // Start HTML
        $html = '<div class="table-responsive"><table cellspacing="0" cellpadding="5"  class="bordered table table-bordered table-striped table-hover" style="width:100%; border-collapse: collapse; border: 1px solid #ddd; font-family: "Segoe UI", Arial, sans-serif;">';

        $html .= '<thead><tr style="background: linear-gradient(to bottom, #ff8c00, #ff7043)">
                    <th rowspan="2" style="color: #fff; vertical-align: middle;background: linear-gradient(to bottom, #ff8c00, #ff7043); font-weight: bold;width: ' . $column_width . '%; text-align: center;font-size: 17px;">NAME</th>
                    <th rowspan="2" style="color: #fff; vertical-align: middle;background: linear-gradient(to bottom, #ff8c00, #ff7043); font-weight: bold;width: ' . $column_width . '%; text-align: center;font-size: 17px;">LEAD ID</th>';

        foreach ($statusOrder as $status) {
            if ($status === 'DISBURSED') {
                $html .= '<th rowspan="2" style="color: #fff;vertical-align: middle;background: linear-gradient(to bottom, #ff8c00, #ff7043); font-weight: bold;width: ' . $column_width . '%; text-align: center;font-size: 15px;">TOTAL<br>TIME</th>';
            } else {
                if ($status === 'AUDIT-NEW' || $status === 'DISBURSAL-NEW') {
                    $html .= '<th style="color: #fff;vertical-align: middle;background: linear-gradient(to bottom, #ff8c00, #ff7043); font-weight: bold;width: ' . ($column_width) . '%; text-align: center;font-size: 15px;">' . $status . '</th>';
                } else {
                    $html .= '<th colspan="2" style="color: #fff;vertical-align: middle;background: linear-gradient(to bottom, #ff8c00, #ff7043); font-weight: bold;width: ' . ($column_width * 2) . '%; text-align: center;font-size: 15px;">' . $status . '</th>';
                }
            }
        }

        $html .= '</tr><tr style="background: linear-gradient(to bottom, #ff8c00, #ff7043)">';

        foreach ($statusOrder as $status) {
            if ($status !== 'DISBURSED') {
                if ($status === 'AUDIT-NEW' || $status === 'DISBURSAL-NEW') {
                    $html .= '<th style="color: #fff;background: linear-gradient(to bottom, #ff8c00, #ff7043);font-weight: bold;width: ' . $column_width . '%; text-align: center;font-size: 17px;">TIME</th>';
                } else {
                    $html .= '<th style="color: #fff;background: linear-gradient(to bottom, #ff8c00, #ff7043);font-weight: bold;width: ' . $column_width . '%; text-align: center;font-size: 17px;">NAME</th><th style="color: #fff;background: linear-gradient(to bottom, #ff8c00, #ff7043);font-weight: bold;width: ' . $column_width . '%; text-align: center;font-size: 17px;">TIME</th>';
                }
            }
        }
        ...
        foreach ($final_report as $lead_id => $statuses) {

            if ($statuses['AUDIT-NEW']['name'] == '') {
                continue;
            }

            $first = reset($statuses);
            $html .= '<tr>';
            $html .= '<td>' . htmlspecialchars($first['name'] ?? '') . '</td>';
            $html .= '<td>' . $lead_id . '</td>';

            foreach ($statusOrder as $status) {
                $bg_clr = 'style="vertical-align: middle"';
                switch ($status) {
                    case 'APPLICATION-RECOMMENDED':
                        $status = 'AUDIT-NEW';
                        break;
                    case 'AUDIT-NEW':
                        $status = 'AUDIT-INPROCESS';
                        break;
                    case 'AUDIT-INPROCESS':
                        $status = 'AUDIT-RECOMMENDED';
                        break;
                    case 'AUDIT-RECOMMENDED':
                        $status = 'SANCTION';
                        break;
                    case 'SANCTION':
                    ...
                    case 'DISBURSAL-NEW':
                        ...
                        break;
                    case 'DISBURSAL-INPROCESS':
                        $status = 'DISBURSE-PENDING';
                        break;
                    case 'DISBURSE-PENDING':
                        $status = 'DISBURSED';
                        break;
                }

                $data = $statuses[$status] ?? null;

                $name = $data['name'] ?? '';
                $hour = $data['hour_diff_prev'] ?? 0;
                $minute = isset($data['minute_diff_prev']) ? str_pad($data['minute_diff_prev'], 2, '0', STR_PAD_LEFT) : 0;
                $time_diff_prev = isset($data['time_diff_prev']) ? str_pad($data['time_diff_prev'], 2, '0', STR_PAD_LEFT) : 0;
                // $sendBack = isset($data['lead_recommend_sendback_flag']) && $data['lead_recommend_sendback_flag'] == 1 ? 'YES' : 'NO';
                $time = ($hour !== '' && $minute !== '') ? "$time_diff_prev" : '';

                if ($hour > 0) {
                    $bg_clr = 'style="font-weight:bold; background-color:rgb(245, 109, 109);vertical-align: middle;"';
                }

                if ($minute >= 20 && ($status == 'AUDIT-INPROCESS' || $status == 'DISBURSE-PENDING' || $status == 'DISBURSED')) {
                    $bg_clr = 'style="font-weight:bold; background-color:rgb(245, 109, 109);vertical-align: middle;"';
                }

                if ($status === 'AUDIT-INPROCESS') {
                    $html .= '<td ' . $bg_clr . '>' . $time . '</td>';
                } elseif ($status === 'DISBURSAL-INPROCESS') {
                    $html .= '<td  ' . $bg_clr . '>' . $time . '</td>';
                } else {
                    $html .= '<td style="vertical-align: middle">' . htmlspecialchars($name) . '</td>';
                    $html .= '<td ' . $bg_clr . '>' . $time . '</td>';
                }

                if ($status === 'DISBURSED') {
                    $html .= '<td style="vertical-align: middle;">' . $data['total_time_per_lead'] . '</td>';
                    break;
                }
            }
